...
export const JobNodeDefinition = defineNode(
    'Job',
    z.object({
        ...
        isBasedInUS: z.boolean().catch(true),
        requireEasternHourAvailability: z.boolean().catch(false),
        status: z.enum(['Open', 'Draft', 'Expired', 'Filled']).catch('Open'),
        externalId: z.string().catch(uuid())
    })
)
    .defineUniqueIndexes(['externalId'])
    .defineRelationship({
